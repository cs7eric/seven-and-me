...
interface DogLoaderProps {
  size?: number; // 1-100 的值，可选参数
}

const DogLoader: React.FC<DogLoaderProps> = ({ size = 50 }) => {
  // 确保size值在1-100范围内
  const normalizedSize = Math.max(1, Math.min(100, size));
  // 使用百分比而不是绝对尺寸，这样更可靠
  const sizePercent = normalizedSize / 100;

  return (
    <div className={'dog-loader-area w-full flex justify-center mt-10 mb-10 '}>
      <StyledWrapper style={{
        transform: `scale(${sizePercent})`,
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
        width: `${normalizedSize * 2}px`,
        height: `${normalizedSize * 2}px`,
        minWidth: '50px',
        minHeight: '50px'
      }}>
        <div className="main">
        ...
        <h3 className={'ml-10 text-2xl text-neutral-100'}>place wait</h3>
