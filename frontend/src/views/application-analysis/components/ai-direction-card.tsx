  ...
  return (
    <div className="space-y-2 border-t border-slate-100 px-3 py-3">
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-3 shadow-[0_2px_8px_rgba(15,23,42,0.04)]">
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] text-slate-600 sm:grid-cols-3">
          <div>
              ...
              return (
                <div key={key} className={`rounded-2xl border p-2 ${tone}`}>
                  ...
                  <div className="mt-1 space-y-1 text-[11px] text-slate-700">
                    {fmt(block.trigger) ? <div>触发：{fmt(block.trigger)}</div> : null}
                    {fmt(block.target) ? <div>目标：{fmt(block.target)}</div> : null}
                    {fmt(block.range) ? <div>区间：{fmt(block.range)}</div> : null}
                  </div>
                </div>
              )
            })}
...
function DateItem({
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_0_rgba(15,23,42,0.04),0_4px_16px_rgba(15,23,42,0.04)]">
      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        className="flex w-full items-center justify-between gap-3 px-3 py-3 text-left transition hover:bg-slate-50/40"
      >
