export function SummaryList({ title, items, tone }: { title: string; items: string[]; tone: "success" | "danger" | "neutral" }) {
  ...
  return (
    <div className={`min-w-0 rounded-2xl border border-slate-200 border-l-4 bg-white p-3 sm:p-4 ${toneClass}`}>
      <div className="mb-3 text-sm font-semibold text-slate-800">{title}</div>
      <div className="space-y-2">
        {items.length ? items.map((item) => (
          <div key={item} className="rounded-xl bg-slate-50 px-3 py-2 text-sm leading-6 break-words text-slate-600">{item}</div>
